...
class BaseModel(ABC):
    ...
    def get_image_paths(self):
        ...
        self.image_paths = []
        for video_object_path in self.video_object_paths:
            img_paths = os.listdir(video_object_path)
            img_paths.sort()
            for img_path in img_paths:
                self.image_paths.append(video_object_path+'/'+img_path)
        return self.image_paths

    def get_image_paths_by_5frame(self):
        """ Return image paths that are used to load current data"""
        self.image_paths = []
        for comp_img_path in self.comp_img_paths:
            self.image_paths += comp_img_path
        return self.image_paths

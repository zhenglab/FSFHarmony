...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        pad_d1 = (window_size[0] - D % window_size[0]) % window_size[0]  #1
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.window_size = window_size
        self.shift_size = tuple(i // 2 for i in window_size)  # [1,4,4]
        self.depth = depth
        self.use_checkpoint = use_checkpoint
...
# from models.deformable.ms_deform_attn import MSDeformAttn


class TemporalRenderCrossMAEShiftPostLN(nn.Module):
    def __init__(self, dim,
        ...
        self.multihead_attn_render = WindowAttention2D(dim=dim, window_size=list(self.window_size)[1], num_heads=num_heads)
    ...
    def forward_part3(self, x, gpt_mask):
        ...
        # attn_mask = get_gpt_mask(D, H, W, window_size[1], shift_size=self.shift_size).to(x.device)
    ...
    def forward_part4(self, x):
        shortcut = x
        x = shortcut + self.drop_path4(self.mlp4(x))
        return self.norm4(x)
    
    def forward(self, x, gpt_mask):
        x = self.forward_part3(x, gpt_mask)
        x = self.forward_part4(x)
        return x



class Temporal3DFourierPostLN(nn.Module):
    def __init__(self, train, dim,
        ...
        if self.num % 2 != 0:
            self.shift_size = list(self.window_size)[1]//2
        self.multihead_attn_render = Fourier3D(train=self.train, dim=dim, window_size=list(self.window_size)[1], num_heads=num_heads)
    ...
    def forward_part3(self, x, gpt_mask):
        ...
        tgt = x_windows.flatten(2,3).permute(0,2,1,3,4,5,6).flatten(2,3)
        B, P, D, Hw, Ww, C = tgt.size()
        
        # attn_mask = get_gpt_mask(D, H, W, window_size[1], shift_size=self.shift_size).to(x.device)
        
        tgt_forward = tgt.flatten(0,1)
        attn_windows = self.multihead_attn_render(tgt_forward,attn_mask)
        attn_windows = attn_windows.view(B,P,D,Hw,Ww,C).transpose(2,1)
        attn_windows = attn_windows.flatten(0,2).view(-1, window_size[0],  window_size[1], window_size[2], C)
        x = window_reverse(attn_windows, window_size, B, Dp, Hp, Wp)  # B D' H' W' C
...
class Fourier3D(nn.Module):
    ...
    def __init__(self, train, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):

        super().__init__()

        self.train = train
        self.mag_weight = nn.Parameter(torch.randn(5,window_size, (window_size//2+1), dim, dtype=torch.float32) * 0.02)
        self.pha_weight = nn.Parameter(torch.randn(5,window_size, (window_size//2+1), dim, dtype=torch.float32) * 0.02)

    def forward_test(self, x, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        ...
        for i in range(int(20/n_frames)):
            ...
            x_tmp = torch.fft.rfftn(x_tmp, s=(5,h,w), dim=(1, 2, 3), norm='ortho')
            mag = torch.abs(x_tmp)
            ...
            x_tmp = torch.fft.irfftn(x_tmp, s=(5,h,w), dim=(1, 2, 3), norm='ortho')
    ...
    def forward_train(self, x, mask=None):
        ...
        """
        B_, t, h, w, C = x.shape
        x = torch.fft.rfftn(x, s=(t,h,w), dim=(1, 2, 3), norm='ortho')
        mag = torch.abs(x)
        pha = torch.angle(x)
        mag_out = mag * self.mag_weight
        pha_out = pha * self.pha_weight
    ...
    def forward(self, x, mask=None):
        if self.train:
            x = self.forward_train(x, mask)
        else:
            x = self.forward_test(x, mask)
        return x

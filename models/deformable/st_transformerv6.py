...
import numpy as np
# from .temporal_deform_attn import MSDeformAttn,MSDeformQAttn,MSDeformOffsetAttn,MSDeformOffsetAttentionWeightAttn, MSDeformMatchAttn,MSDeformMatchV2Attn,MSDeformMatchV3Attn,MSDeformMatchV4Attn
# from .temporal_deform_window import DeformWinAttn,DeformWinAttnV2
from ..lib import swin_transformer_v2

class STTransformerEncoders(nn.Module):
    
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6, dim_feedforward=2048, dropout=0.0,
                 activation="relu", normalize_before=False,
                 return_intermediate_dec=False, n_levels=4, enc_n_points=4, has_flow=False, s_t_type='series', t_attn='v0',window_size=4):
        super().__init__()

        if s_t_type == 'series':
            encoder_layer = STTransformerEncoderLayer(d_model, dim_feedforward,
                                                            dropout, activation,
                                                            n_levels, nhead, enc_n_points, has_flow=has_flow, t_attn=t_attn)
        elif s_t_type == 'swin':
            encoder_layer = SwinTTransformerEncoderLayer(d_model, dim_feedforward,
                                                            dropout, activation,
                                                            n_levels, nhead, enc_n_points, has_flow=has_flow, t_attn=t_attn, window_size=window_size)
        elif s_t_type == 'swin3d':
            encoder_layer = Swin3DTTransformerEncoderLayer(d_model, dim_feedforward,
                                                            dropout, activation,
                                                            n_levels, nhead, enc_n_points, has_flow=has_flow, t_attn=t_attn, window_size=window_size)
        else:
            raise NotImplementedError('Encoder name is not recognized')
            
        self.encoder = STTransformerEncoder(encoder_layer, num_encoder_layers)
...
class STFourierTransformerEncoders(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6, dim_feedforward=2048, dropout=0.0,
        ...
        encoder_layer = SwinTFourierTransformerEncoderLayer(d_model, dim_feedforward,
                                                        dropout, activation,
                                                        n_levels, nhead, enc_n_points, has_flow=has_flow, t_attn=t_attn, window_size=window_size)

            
        self.encoder = STFourierTransformerEncoder(encoder_layer, num_encoder_layers)
        self._reset_parameters()
...
class STTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, has_flow=False, t_attn='v0'):
        super().__init__()
        # cross attention
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
        if t_attn == 'v0':
            self.temporal_attn = DeformWinAttn(d_model, n_levels, n_heads, n_points, has_flow=has_flow)
        elif t_attn == 'v1':
            self.temporal_attn = DeformWinPoolingAttn(d_model, n_levels, n_heads, n_points, has_flow=has_flow)
        elif t_attn == 'v3':
            self.temporal_attn = DeformWinMatchAttn(d_model, n_levels, n_heads, n_points, has_flow=has_flow)
        elif t_attn == 'v4':
            self.temporal_attn = MSDeformMatchV4Attn(d_model, n_levels, n_heads, n_points, has_flow=has_flow)
        elif t_attn == 'none':
    ...
    def forward(self, src, pos, padding_mask=None, flow=None):
        # self attention
        ...
        src2 = self.spatial_attn(q, k, src)[0]
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        if self.temporal_attn != None:
            ...
            if pos is None:
                ...
                # tgt = self.with_pos_embed(src, pos)
                tgt = src
            src2, sampling_locations = self.temporal_attn(tgt, src, padding_mask, flow=flow, pos=pos)
...
class SwinTTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
        ...
        self.temporal_attn = None
        # if t_attn == 'v0':
        #     self.temporal_attn = DeformWinAttn(d_model, n_levels, n_heads, n_points, has_flow=has_flow, window_size=window_size)
        #     # self.dropout1 = nn.Dropout(dropout)
        #     self.norm1 = nn.LayerNorm(d_model)
        # elif t_attn == 'v2':
        #     self.temporal_attn = DeformWinAttnV2(d_model, n_levels, n_heads, n_points, has_flow=has_flow, window_size=window_size)
        #     # self.dropout1 = nn.Dropout(dropout)
        #     self.norm1 = nn.LayerNorm(d_model)
        # else:
        #     self.temporal_attn = None
        ...
        self.spatial_attn = swin_transformer_v2.BasicLayer(d_model,(self.input_resolution,self.input_resolution), depth=2, num_heads=8, window_size=8, mlp_ratio=2.0,qkv_bias=True,drop_path=0.1,
    ...
    def forward(self, src, pos, padding_mask=None, flow=None):
        ...
        if self.temporal_attn is not None:
            src = src.view(-1, self.n_levels, L, C).flatten(1,2)
            tgt = src
            src2, sampling_locations = self.temporal_attn(tgt, src, padding_mask, flow=flow, pos=pos)
            src = src + self.dropout1(src2)
            src = self.norm1(src)
            src = src.view(-1,self.n_levels, L, C).flatten(0,1)
        else:
            sampling_locations = None
        # ffn
        src = self.forward_ffn(src)
...
class SwinTFourierTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, has_flow=False, t_attn='v0',window_size=4):
        ...
        self.spatial_attn = swin_transformer_v2.BasicFourierLayer(d_model,(self.input_resolution,self.input_resolution), depth=2, num_heads=8, window_size=16, mlp_ratio=2.0,qkv_bias=True,drop_path=0.1,
                    norm_layer=norm_layer)
...
class Swin3DTTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, has_flow=False, t_attn='v0',window_size=4):
    ...
    def forward(self, src, pos, padding_mask=None, flow=None):
        ...
        B, L, C = src.size()
        h=w=64
        src = src.transpose(2,1).view(B, C, h, w).view(-1, self.n_levels, C, h, w).transpose(2,1) #b c d h w
        src2 = self.spatial_attn(src) #b c d h w
        
        src = src.flatten(3).permute(0,2,3,1)
        src2 = src2.flatten(3).permute(0,2,3,1)
        src = self.norm0(src+self.linear0(src2))
        
        # src = src2.flatten(3).permute(0,2,3,1)
        if self.temporal_attn is not None:
            src = src.view(-1, self.n_levels, L, C).flatten(1,2)
            tgt = src
            src2, sampling_locations = self.temporal_attn(tgt, src, padding_mask, flow=flow, pos=pos)
...
class STTransformerEncoder(nn.Module):
    ...
    def forward(self, src, pos_embeds=None, padding_mask=None, flow=None):
        ...
        for _, layer in enumerate(self.layers):
            output,sampling_locations = layer(output, pos_embeds,flow=flow)
        return output,sampling_locations

class STFourierTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers

...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None, flow=None):
        ...
        value = value.view(N, Len_in, self.n_heads, self.d_model // self.n_heads)  # [B, T*H*W, 1, 256]
        if flow is None:
            sampling_offsets = self.sampling_offsets(query).view(N, Len_q, self.n_heads, self.n_levels, self.n_points, 2)  # [B, T*H*W, 1, 5, 16, 2]
        else:
            ...
            h = w = 64
            sampling_offsets = self.sampling_offsets(query).view(N, Len_q, self.n_heads, self.n_points, 2).unsqueeze(3)
            flow = flow.permute(0,1,4,5,2,3).flatten(1,3).unsqueeze(2).unsqueeze(4).repeat(1, 1, self.n_heads, 1, self.n_points, 1)
            sampling_offsets = sampling_offsets+flow
        ...
        attention_weights = F.softmax(attention_weights, -1).view(N, Len_q, self.n_heads, self.n_levels, self.n_points)  # [B, T*H*W, 1, 5, 16]
        # attention_weights = self.attention_weights.repeat(N, Len_q, 1,1,1).to(query.device)
        # N, Len_q, n_heads, n_levels, n_points, 2
        if reference_points.shape[-1] == 2:
            offset_normalizer = torch.stack([input_spatial_shapes[..., 1], input_spatial_shapes[..., 0]], -1)
            sampling_locations = reference_points[:, :, None, :, None, :] \
                                 + sampling_offsets / offset_normalizer[None, None, None, :, None, :]  # [B, H*W*T, 1, T, 16, 2]
...
class MSDeformMatchV3Attn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None, flow=None,pos=None):
        ...
        attention_weights = attention_weights.div(9.0).unsqueeze(-1).repeat(1,1,1,1,1,9).flatten(4)
